                ...
                <div class="card-body">
                    <form action="Controlador?menu=Cliente" method="POST"> <!-- FORMULARIO PARA INGRESAR UN NUEVO CLIENTE -->
                        ...
                        <div class="form-group">
                            <label>Dni</label>
                            <input type="text" value="${not empty cliente ? cliente.getDni() : ''}" name="txtDni" class="form-control"> 
                        ...
                        <div class="form-group">
                            <label>Nombres</label>
                            <input type="text" value="${not empty cliente ? cliente.getNom() : ''}" name="txtNombres" class="form-control"> 
                        </div>
                        <!-- Campo para el Telefono -->
                        <div class="form-group">
                            <label>Telefono</label>
                            <input type="text" value="${not empty cliente ? cliente.getTel() : ''}" name="txtTel" class="form-control"> 
                            <!-- Mensaje de error para el campo Telefono -->
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" value="${not empty cliente ? cliente.getDir() : ''}" name="txtDir" class="form-control"> 
                        </div>
                        <!-- Campo para el Estado -->
                        <div class="form-group">
                            <label>Estado</label>
                            <input type="text" value="${not empty cliente ? cliente.getEstado() : ''}" name="txtEstado" class="form-control">
                        ...
                        <div class="d-flex">
                            <!-- Mostrar el botón de "Guardar" solo cuando no se está editando un cliente -->
                            <c:if test="${empty editarCliente}">
                                <button type="submit" name="accion" style="margin-right: 6px" value="Guardar" class="btn btn-primary" onclick="return confirm('¿Estás seguro de AGREGAR este cliente?')">Guardar</button>
                                <button type="submit" name="accion" value="cancelarGuardado" class="btn btn-danger" onclick="return confirm('¿Estás seguro de CANCELAR el guardado del cliente?')">Cancelar</button>
                            </c:if>
                            <!-- Mostrar los botones de "Actualizar" y "Cancelar" solo cuando se está editando un cliente -->
                            <c:if test="${not empty editarCliente}">
                                <button type="submit" name="accion" style="margin-right: 6px" value="Actualizar" class="btn btn-success" onclick="return confirm('¿Estás seguro de ACTUALIZAR este cliente?')">Actualizar</button>
                        ...
                        <c:forEach var="cl" items="${clientes}"> <!--El atributo clientes se esta enviando desde el controlador.-->
                            <tr> <!-- ARROJAMIENTO DE LOS DATOS DE LOS CLIENTES AGREGADOS -->
                                ...
                                <td class="d-flex"> <!-- td PARA LOS BOTONES DE EDITAR Y ELIMINAR CLIENTES -->
                                    <a class="btn btn-warning" href="Controlador?menu=Cliente&accion=Editar&id=${cl.getId()}" onclick="return confirm('¿Estás seguro de iniciar a EDITAR este cliente?')">Editar</a> 
                                    <a class="btn btn-danger" style="margin-left: 6px" href="Controlador?menu=Cliente&accion=Delete&id=${cl.getId()}" onclick="return confirm('¿Estás seguro de ELIMINAR este cliente?')">Eliminar</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
